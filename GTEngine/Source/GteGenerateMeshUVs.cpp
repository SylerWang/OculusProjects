...
std::string const gGenerateMeshUVsHLSLString = GTE_MAKE_HLSL_STRING(
cbuffer Bounds
{
    int2 bound;
...
#else
// WARNING: The GTE_MAKE_HLSL_STRING macro does not work with gcc.
// The compiler generates errors about 'cbuffer', so the macro is
// not generating a correctly formed string for that compiler.
#endif
