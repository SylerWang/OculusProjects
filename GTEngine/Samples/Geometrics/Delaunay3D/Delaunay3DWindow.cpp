...
Delaunay3DWindow::~Delaunay3DWindow()
{
    ...
    if (mSolidTetra[0])
    {
        UnsubscribeCW(mSolidTetra[0]);
    }
    else
    {
        UnsubscribeCW(mWireTetra[0]);
    }
    Unsubscribe(mScene);
}
//----------------------------------------------------------------------------
...
bool Delaunay3DWindow::CreateScene()
{
    ...
    Subscribe(mWireTetra[0]->worldTransform,
...
void Delaunay3DWindow::CreateSphere()
{
    ...
    mSphere->Update();
    Subscribe(mSphere->worldTransform, effect->GetPVWMatrixConstant());
